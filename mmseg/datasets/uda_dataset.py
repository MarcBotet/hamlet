...
from . import CityscapesDataset, ShiftDataset
from .builder import DATASETS


def get_rcs_class_probs(data_root, temperature):
    dataset_name = data_root.split('/')[-2]  # data_root is /data/dataset/<name>/
    data_sample_class_root = f'data/{dataset_name}'
    with open(osp.join(data_sample_class_root, 'sample_class_stats.json'), 'r') as of:
...
class UDADataset(object):
    ...
    def __init__(self, source, target, cfg):
        ...
        if self.rcs_enabled:
            ...
            dataset_name = cfg['source']['data_root'].split('/')[-2]
            data_sample_class_root = f'data/{dataset_name}'
            with open(
                    osp.join(data_sample_class_root,
            ...
            for c in self.rcs_classes:
                ...
                for file, pixels in samples_with_class_and_n[c]:
                    if pixels > self.rcs_min_pixels:
                        if isinstance(self.source, ShiftDataset):
                            name_file = '/'.join(file.split('/')[-2:])
                        else:
                            name_file = file.split('/')[-1]
                        self.samples_with_class[c].append(name_file)
